// src/lib/antidoc-state/antidoc-state.js
const path = require("path");
const fs = require("fs");

const DATABASE_DIR = path.join(__dirname, '..', '..', '..', 'database', 'antidoc');
...
function definirEtapaAntidoc(groupId, userId, etapa, dados = {}) {
...
function obterEtapaAntidoc(groupId, userId) {
    const etapas = carregarEtapas();
    const chave = `${groupId}_${userId}`;
...
function atualizarEtapaAntidoc(groupId, userId, etapa, dados) {
    const etapas = carregarEtapas();
    const chave = `${groupId}_${userId}`;
    if (etapas[chave]) {
...
function limparEtapaAntidoc(groupId, userId) {
    const etapas = carregarEtapas();
    const chave = `${groupId}_${userId}`;
    if (etapas[chave]) {
        delete etapas[chave];
...
function incrementarAdvertenciaAntidoc(groupId, userId) {
    const configs = carregarConfigs();
...
function resetarAdvertenciasAntidoc(groupId, userId) {
    const configs = carregarConfigs();
...
module.exports = {
    ...
    definirEtapaAntidoc,
    obterEtapaAntidoc,
    atualizarEtapaAntidoc,
    limparEtapaAntidoc,
    incrementarAdvertenciaAntidoc,
    resetarAdvertenciasAntidoc
};
