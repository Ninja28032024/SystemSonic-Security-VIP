// src/lib/antipayment-state/antipayment-state.js
const path = require("path");
const fs = require("fs");

const DATABASE_DIR = path.join(__dirname, '..', '..', '..', 'database', 'antipayment');
...
function definirEtapaAntipayment(groupId, userId, etapa, dados = {}) {
...
function obterEtapaAntipayment(groupId, userId) {
    const etapas = carregarEtapas();
    const chave = `${groupId}_${userId}`;
...
function atualizarEtapaAntipayment(groupId, userId, etapa, dados) {
    const etapas = carregarEtapas();
    const chave = `${groupId}_${userId}`;
    if (etapas[chave]) {
...
function limparEtapaAntipayment(groupId, userId) {
    const etapas = carregarEtapas();
    const chave = `${groupId}_${userId}`;
    if (etapas[chave]) {
        delete etapas[chave];
...
function incrementarAdvertenciaAntipayment(groupId, userId) {
    const configs = carregarConfigs();
...
function resetarAdvertenciasAntipayment(groupId, userId) {
    const configs = carregarConfigs();
...
module.exports = {
    obterConfigAntipayment,
    salvarConfigAntipayment,
    desativarAntipayment,
    definirEtapaAntipayment,
    obterEtapaAntipayment,
    atualizarEtapaAntipayment,
    limparEtapaAntipayment,
    incrementarAdvertenciaAntipayment,
    resetarAdvertenciasAntipayment
};
