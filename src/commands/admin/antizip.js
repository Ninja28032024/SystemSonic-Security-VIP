// src/commands/admin/antizip.js
const path = require("path");
const { isUserAdmin, isOwner } = require(path.join(__dirname, '..', '..', 'utils.js'));
const { limparEtapaAntizip, definirEtapaAntizip, obterConfigAntizip, desativarAntizip } = require(path.join(__dirname, '..', '..', 'lib', 'antizip-state', 'antizip-state.js'));
...
module.exports = {
    ...
    async execute(sock, m, options) {
        ...
        try {
            ...
            definirEtapaAntizip(from, sender, 'intro', { configuredBy: sender });

            await sock.sendMessage(from, {
                text: "*OK!* VAMOS CONFIGURAR O ANTI ZIP\n\nDeseja que o membro que postar o arquivo ZIP seja removido?",
                footer: "SYSTEM-SONIC - Security Anti ZIP",
                interactiveButtons: [
                    { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "SIM", id: "antizip_intro_sim" }) },
                    { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "NÃO", id: "antizip_intro_nao" }) }
                ]
            }, { quoted: m });

        } catch (error) {
            console.error("Erro antizip:", error.message);
            await sock.sendMessage(from, { text: "*Ops!* Erro ao executar comando antizip." }, { quoted: m });
        }
    }
};
