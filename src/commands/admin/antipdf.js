// src/commands/admin/antipdf.js
const path = require("path");
const { isUserAdmin, isOwner } = require(path.join(__dirname, '..', '..', 'utils.js'));
const { limparEtapaAntipdf, definirEtapaAntipdf, obterConfigAntipdf, desativarAntipdf } = require(path.join(__dirname, '..', '..', 'lib', 'antipdf-state', 'antipdf-state.js'));
    ...
    async execute(sock, m, options) {
        ...
        try {
            ...
            if (configAtual && configAtual.ativo) {
                ...
                const status = `*ANTI PDF - SYSTEMSONIC SECURITY*\n\n— dados do sistema de anti PDF —\n*Deletar a mensagem?* ${deletarMsg}\n*Remover membro?* ${removerMembro}\n*Remoção:* ${modoRemocaoTexto}\n\nAnti PDF - SystemSonic Security`;

                await sock.sendMessage(from, {
                    text: status,
                    footer: "SYSTEM-SONIC - Security Anti PDF",
                    interactiveButtons: [
                        { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "DESATIVAR", id: "antipdf_desativar" }) }
                    ]
                }, { quoted: m });
                return;
            }

            limparEtapaAntipdf(from, sender);
            definirEtapaAntipdf(from, sender, 'intro', { configuredBy: sender });

            await sock.sendMessage(from, {
                text: "*OK!* VAMOS CONFIGURAR O ANTI PDF\n\nDeseja que o membro que postar o PDF seja removido?",
                footer: "SYSTEM-SONIC - Security Anti PDF",
                interactiveButtons: [
                    { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "SIM", id: "antipdf_intro_sim" }) },
                    { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "NÃO", id: "antipdf_intro_nao" }) }
                ]
            }, { quoted: m });

        } catch (error) {
            console.error("Erro antipdf:", error.message);
            await sock.sendMessage(from, { text: "*Ops!* Erro ao executar comando antipdf." }, { quoted: m });
        }
    }
};
