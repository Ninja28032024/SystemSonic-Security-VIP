// src/commands/admin/antifig.js
const path = require("path");
const { isUserAdmin, isOwner } = require(path.join(__dirname, '..', '..', 'utils.js'));
const { limparEtapaAntifig, definirEtapaAntifig, obterConfigAntifig, desativarAntifig } = require(path.join(__dirname, '..', '..', 'lib', 'antifig-state', 'antifig-state.js'));

module.exports = {
    name: "antifig",
    aliases: ["antisticker", "antifigurinha"],
    async execute(sock, m, options) {
        ...
        try {
            ...
            if (configAtual && configAtual.ativo) {
                ...
                const status = `*ANTI FIGURINHAS - SYSTEMSONIC SECURITY*\n\n— dados do sistema de anti figurinhas —\n*Deletar a mensagem?* ${deletarMsg}\n*Remover membro?* ${removerMembro}\n*Remoção:* ${modoRemocaoTexto}\n\nAnti figurinhas - SystemSonic Security`;

                await sock.sendMessage(from, {
                    text: status,
                    footer: "SYSTEM-SONIC - Security Anti Figurinhas",
                    interactiveButtons: [
                        { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "DESATIVAR", id: "antifig_desativar" }) }
                    ]
                }, { quoted: m });
                return;
            }

            limparEtapaAntifig(from, sender);
            definirEtapaAntifig(from, sender, 'intro', { configuredBy: sender });

            await sock.sendMessage(from, {
                text: "*OK!* VAMOS CONFIGURAR O ANTI FIGURINHAS\n\nDeseja que o membro que postar a figurinha seja removido?",
                footer: "SYSTEM-SONIC - Security Anti Figurinhas",
                interactiveButtons: [
                    { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "SIM", id: "antifig_intro_sim" }) },
                    { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "NÃO", id: "antifig_intro_nao" }) }
                ]
            }, { quoted: m });

        } catch (error) {
            console.error("Erro antifig:", error.message);
            await sock.sendMessage(from, { text: "*Ops!* Erro ao executar comando antifig." }, { quoted: m });
        }
    }
};